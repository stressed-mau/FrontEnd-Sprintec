...
export function EducationDuplicateRegistrationModal({ title, message, closeLabel = "Entendido", onClose }: {
  title: string
  message: string
  closeLabel?: string
  onClose: () => void
  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm">
      <div className="relative w-full max-w-md rounded-[32px] bg-white p-8 shadow-2xl">
        <button type="button" onClick={onClose} className="absolute right-5 top-5 text-gray-400 transition-colors hover:text-gray-600" aria-label={`Cerrar alerta de ${title.toLowerCase()}`}>
          <X className="size-6" />
        </button>
