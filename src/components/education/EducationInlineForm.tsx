...
const DEGREE_OPTIONS = ["Bachillerato", "Técnico Superior", "Técnico Profesional", "Licenciatura", "Ingeniería", "Grado", "Maestría", "Master", "Doctorado", "Posgrado", "Diplomado", "Certificación Profesional"]
  ...
  return (
    <form onSubmit={onSubmit} className={formClassName} noValidate>
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <EducationFieldError label="Institución académica" id="education-institution" error={errors.company} required>
          <Input id="education-institution" maxLength={100} value={formData.company} disabled={isSaving} onBlur={() => onBlur("company")} onChange={(event) => onFieldChange("company", event.target.value)} placeholder="Ej: Universidad Mayor de San Andrés" aria-invalid={Boolean(errors.company)} className={inputClassName(Boolean(errors.company))} />
        </EducationFieldError>

        <EducationFieldError label="Nivel de formación" id="education-title" error={errors.position} required>
          <SearchableSelect options={degreeOptions} placeholder="Busca y selecciona un nivel de formación" id="education-title" value={formData.position} disabled={isSaving} onBlur={() => onBlur("position")} onChange={(event) => onFieldChange("position", event.target.value)} aria-invalid={Boolean(errors.position)} className={`h-10 w-full rounded-md border bg-white px-3 text-sm outline-none transition ${getSelectFocusClassName(Boolean(errors.position), isModalTone)}`} />
        </EducationFieldError>
      </div>

      <EducationFieldError label="Área de estudio" id="education-field" error={errors.fieldOfStudy} required>
        <SearchableSelect options={fieldOptions} placeholder="Busca y selecciona un área de estudio" id="education-field" value={formData.fieldOfStudy} disabled={isSaving} onBlur={() => onBlur("fieldOfStudy")} onChange={(event) => onFieldChange("fieldOfStudy", event.target.value)} aria-invalid={Boolean(errors.fieldOfStudy)} className={`h-10 w-full rounded-md border bg-white px-3 text-sm outline-none transition ${getSelectFocusClassName(Boolean(errors.fieldOfStudy), isModalTone)}`} />
      </EducationFieldError>

      <div className="space-y-2">
        <Label htmlFor="education-description" className="text-sm font-medium text-gray-700">Descripción</Label>
        <Textarea id="education-description" value={formData.description} disabled={isSaving} rows={4} maxLength={300} onBlur={() => onBlur("description")} onChange={(event) => onFieldChange("description", event.target.value)} placeholder="Describe tu formación académica, logros o especializaciones..." aria-invalid={Boolean(errors.description)} className={`resize-none ${inputClassName(Boolean(errors.description))}`} />
        {errors.description ? <p className="text-sm text-red-600">{errors.description}</p> : null}
      </div>

      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <EducationFieldError label="Fecha de emisión" id="education-end-date" error={errors.endDate}>
      ...
      <div className="space-y-2">
        <Label htmlFor="education-document" className="text-sm font-medium text-gray-700">Documento de formación</Label>
        <div className="space-y-3">
          {formData.certificate ? <EducationCertificatePreview source={formData.certificate} canRemove={canRemoveCertificate} isSaving={isSaving} onRemove={onRemoveCertificate} /> : null}
