...
type ExperienceSearchableSelectProps = {
  id: string
  value: string
  disabled?: boolean
  options: string[]
...
export function ExperienceSearchableSelect({ id, value, disabled, options, placeholder, className, onBlur, onChange }: ExperienceSearchableSelectProps) {
  ...
  return (
    <div className="relative">
      <Input id={id} value={value} disabled={disabled} onFocus={() => setIsOpen(true)} onBlur={() => {
      ...
      {isOpen ? <ExperienceOptions options={filteredOptions} activeIndex={activeIndex} onSelect={selectOption} onHover={setActiveIndex} /> : null}
    </div>
  )
}

function ExperienceOptions({ options, activeIndex, onSelect, onHover }: { options: string[]; activeIndex: number; onSelect: (option: string) => void; onHover: (index: number) => void }) {
  return (
    <div className="absolute z-30 mt-1 max-h-44 w-full overflow-y-auto rounded-lg border border-gray-200 bg-white shadow-xl">
      {options.length ? options.map((option, index) => (
        <button key={option} type="button" onMouseDown={(event) => {
          ...
          onSelect(option)
        }} onMouseEnter={() => onHover(index)} className={`block w-full px-3 py-2 text-left text-sm text-[#003A6C] transition-colors ${index === activeIndex ? "bg-blue-50" : "hover:bg-blue-50"}`}>
          {option}
        </button>
...
function handleOptionsKey(event: KeyboardEvent<HTMLInputElement>, options: string[], activeIndex: number, setActiveIndex: (updater: (current: number) => number) => void, selectOption: (option: string) => void, isOpen: boolean, setIsOpen: (isOpen: boolean) => void) {
