...
export function ExperienceAttachmentFields({
  ...
  return (
    <div className="space-y-2">
      ...
      <input id="experience-image" ref={fileInputRef} type="file" accept=".jpg,.jpeg,.png,image/jpeg,image/png" disabled={isSaving} onChange={onImageChange} className="hidden" />
      ...
      {preview ? <ImagePreview preview={preview} canRemove={canRemoveImage} isSaving={isSaving} onRemove={onRemoveImage} /> : null}
...
function ImagePreview({ preview, canRemove, isSaving, onRemove }: { preview: string; canRemove: boolean; isSaving: boolean; onRemove: () => void }) {
  return (
    <div className="mt-2 flex flex-wrap items-center gap-3">
      <img src={preview} alt="Vista previa" className="size-20 rounded-lg object-cover shadow-sm" />
      {canRemove ? (
