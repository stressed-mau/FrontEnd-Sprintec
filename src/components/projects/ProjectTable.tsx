...
function ProjectEmptyState({ emptyMessage, isEditVariant }: { emptyMessage: string; isEditVariant: boolean }) {
  ...
  return (
    <div className={className}>
      <div className="mx-auto mb-4 flex size-16 items-center justify-center rounded-full bg-[#D9EAF4]">
        <FolderGit2 className="size-8 text-[#003A6C]" />
      </div>
      <h3 className="mb-1 text-sm font-medium text-[#003A6C]">No hay proyectos</h3>
      <p className="text-sm text-[#4B778D]">{emptyMessage}</p>
    </div>
  )
}
...
function ProjectTableRow({ project, selectable, selectedIds, onToggleSelect, onRowClick, onEdit, isEditVariant }: ProjectTableRowProps) {
  return (
    <tr key={project.id} onClick={() => onRowClick?.(project)} className={getRowClassName(isEditVariant, Boolean(onRowClick))}>
      ...
      <td className="px-4 py-3 text-[#4B778D]">{formatProjectDate(project.fechaInicio)} - {project.is_current ? "Actualidad" : formatProjectDate(project.fechaFin)}</td>
      <td className="px-4 py-3"><ProjectStatusBadge project={project} isEditVariant={isEditVariant} /></td>
      {onEdit ? <ProjectEditCell project={project} onEdit={onEdit} /> : null}
    </tr>
  )
}
...
function ProjectIdentityCell({ project, isEditVariant }: { project: ProjectItem; isEditVariant: boolean }) {
  return (
    <div className="flex min-w-0 items-center gap-3">
      ...
      <p className={isEditVariant ? "min-w-0 break-words font-medium text-[#003A6C]" : "min-w-0 break-words font-semibold text-[#003A6C]"}>
        {project.nombre}
      </p>
    </div>
  )
...
function ProjectStatusBadge({ project, isEditVariant }: { project: ProjectItem; isEditVariant: boolean }) {
  const className = isEditVariant
    ? project.is_current ? "bg-[#D9EAF4] text-[#003A6C] hover:bg-[#D9EAF4]" : "bg-[#EEF5F9] text-[#4B778D] hover:bg-[#EEF5F9]"
...
function ProjectEditCell({ project, onEdit }: { project: ProjectItem; onEdit: (project: ProjectItem) => void }) {
  return (
    <td className="px-4 py-3 text-right" onClick={(event) => event.stopPropagation()}>
      <Button type="button" variant="outline" onClick={() => onEdit(project)} className="h-9 border-[#A5D7E8] bg-white text-[#003A6C] hover:bg-[#EEF5F9]">
...
function getTableWrapperClassName(isEditVariant: boolean) {
  if (isEditVariant) return "overflow-hidden rounded-2xl border border-[#A5D7E8] bg-white shadow-sm"
...
function getCheckboxClassName(isEditVariant: boolean) {
  return isEditVariant ? "size-4 rounded-none border-[#A5D7E8]" : "size-4 rounded-none border-[#A5D7E8]"
}

function getRowClassName(isEditVariant: boolean, canClick: boolean) {
  if (isEditVariant) return `border-t border-[#D7E6F2] transition ${canClick ? "cursor-pointer hover:bg-[#F7FBFD]" : ""}`
...
function getTechnologyBadgeClassName(isEditVariant: boolean) {
  return isEditVariant ? "bg-[#D9EAF4] text-[#003A6C] hover:bg-[#D9EAF4]" : "bg-[#D9EAF4] text-[#003A6C]"
}
