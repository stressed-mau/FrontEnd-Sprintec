...
import type { ExperienceDto, ExperienceItem, ExperienceType } from "@/types/experience"
...
export function normalizeExperience(dto: ExperienceDto, index: number, typeHint?: ExperienceType): ExperienceItem {
  ...
  return {
    ...
    certificate: getCertificateUrl(dto),
  }
}
...
function getCompanyName(dto: ExperienceDto) {
  return asString(
...
function normalizeType(value: unknown): ExperienceType {
  ...
  const academicTypes = ["academica", "academico", "academic", "education", "educacion", "formacion", "study", "estudio"]
  return academicTypes.includes(normalizedValue) ? "academica" : "laboral"
}
