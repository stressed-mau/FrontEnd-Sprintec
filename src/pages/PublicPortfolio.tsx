...
// Dentro de MyPortfolio.tsx
const mapToVisibilityData = (portfolio: Portfolio) => ({
  projects: portfolio.projects.map((p) => ({
    id: Number(p.id), 
    label: p.nombre ?? "",
    sublabel: p.descripcion ?? "",
    checked: p.is_current ?? true, // O la propiedad que uses para visibilidad
  })),
  skills: portfolio.skills.map((s) => ({
    id: Number(s.id),
    label: s.name ?? "",
    sublabel: s.level ?? "",
    checked: true, // Aquí podrías mapear s.is_public si el back lo envía
  })),
  ...
  networks: portfolio.socialNetworks.map((n) => ({
    id: Number(n.id),
    label: n.name ?? "",
    sublabel: n.url ?? "",
    checked: true,
